...
public:
    ...
    comm(bool cartesian = false) {
        MPI_Comm_size(MPI_COMM_WORLD, &m_size);
        MPI_Comm_rank(MPI_COMM_WORLD, &m_rank);
        if(cartesian){
            MPI_Comm newcomm;
            int dims [2] = {0, 0}, periodic [] = {true, true};
            MPI_Dims_create(m_size, 2, dims);
            m_x_size = dims[0];
            m_y_size = dims[1];
            MPI_Cart_create(MPI_COMM_WORLD, 2, dims, periodic, false, &newcomm);
            mpi_comm = newcomm;
        } else {
...
public:
  ...
  /// Returns the rank of this process in the communicator
    ...
    int get_rank(int x, int y) {
        int dims[2] = {x, y};
        int rank = 0;
        MPI_Cart_rank(mpi_comm, dims, &rank);
        return rank;
    }

    std::pair<int, int> get_coordinates(int rank){
        int dims [2] = {0, 0};
        MPI_Cart_coords(mpi_comm, rank, 2, dims);
        return std::make_pair(dims[0], dims[1]);
    }

    std::pair<int, int> get_current_coordinates(){
        return get_coordinates(m_rank);
    }
